...
import { SubscriptionPlanEnum } from "@/src/types";
import { Colors } from "@/src/utils/constant/colors";
import { Check, Crown } from "lucide-react-native";
import React, { useEffect, useState } from "react";
import { ActivityIndicator, Pressable, View } from "react-native";
import { Button } from "react-native-paper";
import { useCreateSubscription, useGetProducts } from "../hooks/subscription";
...
> = ({ handleClose, title, description, feature }) => {
  const isDark = useIsDarkTheme();
  ...
  const handleSubscribe = async () => {
    ...
    try {
      await createSubscription.mutateAsync({
        ...
        interval: "month",
        plan: isBasicPlan ? SubscriptionPlanEnum.BASIC : isStandardPlan ? SubscriptionPlanEnum.STANDARD : SubscriptionPlanEnum.PREMIUM,
      });
      handleClose();
    } catch (error) {
    ...
    <View className="h-[80vh]">
      ...
      <View className="flex flex-row gap-3 mt-4">
        ...
        <Button
          ...
          disabled={
            !selectedProduct || isLoading || !products || products?.length === 0
          }
